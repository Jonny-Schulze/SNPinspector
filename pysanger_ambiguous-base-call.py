...
def generate_consensusseq(abidata, ratio_threshold=0.8):
    consensus_seq = "" 
    
    for values in zip(abidata["channel"]["A"], abidata["channel"]["T"], abidata["channel"]["G"], abidata["channel"]["C"]):
    # Check if the sum of values is zero
        if sum(values) == 0:
            consensus_seq += 'N'
        else:
    # Calculate the ratios based on individual values
            ratios = [v / sum(values) for v in values]

    # Check if all ratios are below the threshold
            if all(ratio < ratio_threshold for ratio in ratios):
                consensus_seq += 'N'
            else:
    # If any ratio is above the threshold, assign the corresponding base
                consensus_seq += _atgc_dict[values.index(max(values))]
     
    return (consensus_seq, consensus_seq.translate(str.maketrans("ATGCN","TACGN"))[::-1]) 
...
def visualize(abidata, template=None, strand=1, fig=None, region="all"):  
    ...
    if strand == -1:
        ...
        min_length = min(len(avalues), len(tvalues), len(gvalues), len(cvalues))
        # Reverse each list up to the minimum length
        avalues = avalues[:min_length][::-1]
        tvalues = tvalues[:min_length][::-1]
        gvalues = gvalues[:min_length][::-1]
        cvalues = cvalues[:min_length][::-1]
   
    if template is not None:
    #updated to use "PairwiseAligner"
